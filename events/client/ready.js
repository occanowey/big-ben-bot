...
module.exports = (client, config, db) =>{
	...
	const disbot = new disbotapi(client.user.id, "token-here", false) // BOOLEAN IS FOR DEBUG MODE
        ...
        client.guilds.cache.forEach(async g => {
            await db.query(`SELECT * FROM guilds WHERE id='${g.id}'`, async (err, row) => {
                if(err) throw err;
                if(!row[0]) {
                    await db.query(`INSERT INTO guilds (id, chan, logs) VALUES ('${g.id}', 'none', 'none')`, async (err, row) => {
                        if(err) throw err;
                    });
                }
            });
        ...
        setInterval(() => {
            ...
            bigFuckingBen(client, config, moment, fs, ms, db, bigdogstatus, looper)
        ...
        async function bigFuckingBen(client, config, moment, fs, ms, db, bigdogstatus, looper) {
            ...
            if(datetime.includes(`:00`)) {
                getDicked(client, config, moment, fs, ms, db, bigdogstatus, looper)
            }

        };

        async function getDicked(client, config, moment, fs, ms, db, bigdogstatus, looper) {
            try {
                // Grab all guilds that have setup the bot
            await db.query(`SELECT * FROM guilds WHERE chan!='none'`, async (err, rows) => {
                if(err) throw err;
                            ...
                            if(bigdogstatus != undefined) {
                                try {
                                    ...
                                    if(bigdogstatus.members.size >= 1) {
                                        ...
                                        connector(client, config, bigdogstatus, fs, ms, db, data);
        ...
        async function connector(client, config, bigdogstatus, fs, ms, db, data) {
            ...
            await bigdogstatus.join().then(async connection => {
                ...
                if(e) {
                    ...
                    await db.query(`SELECT * FROM guilds WHERE id='${data.id}'`, async (err, row) => {
                        if(err) throw err;
                        if(row[0]) {
                            try {
                                await db.query(`UPDATE guilds SET chan='none' WHERE id='${data.id}'`, async (err, row) => {
